...
class Solution:
    def characterReplacement(self, s: str, k: int) -> int:
        ...
        for r in range(len(s)):
            w_dict[s[r]] += 1 # Increment dict with right ptr
            maxf = max(maxf, w_dict[s[r]]) # calculate window
            while (r-l+1) - maxf > k: # Keep growing window till wind size - most common <=k
                w_dict[s[l]] -= 1 # if its >k, then decrement and move left operator
                l = l + 1
            res = max(res, r-l+1) 
        return res
